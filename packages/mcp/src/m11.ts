...
export type McpFamily = 'agents' | 'tasks' | 'scheduler' | 'workers' | 'swarm' | 'memory' | 'learning' | 'sandbox' | 'security' | 'policy' | 'providers' | 'models' | 'workflows' | 'evaluation' | 'federation' | 'system' | 'github' | 'filesystem' | 'browser' | 'events' | 'intelligence' | 'control';
...
export class McpCapabilityBridge {
  ...
  async dispatch(family: McpFamily, action: string, input: Record<string, unknown>, context: McpCallContext): Promise<unknown> {
    ...
    if (family === 'control') return this.control(action, input);
  ...
  private async control(action: string, input: Record<string, unknown>): Promise<unknown> {
    const control = this.runtime.controlPlane;
    if (action === 'status') return control.snapshot();
    if (action === 'health') return control.health();
    if (action === 'metrics') return control.metrics.snapshot();
    if (action === 'events') return control.listEvents({ limit: typeof input.limit === 'number' ? input.limit : 100 });
    if (action === 'trace') return control.trace(String(input.executionId ?? input.id ?? ''));
    if (action === 'sessions') return control.sessions.list();
    if (action === 'doctor') return control.doctor.run();
...
const familyActions: Record<McpFamily, string[]> = {
  ...
  federation: ['nodes', 'nodes_list', 'node_status', 'node_register', 'node_drain', 'node_remove', 'capabilities', 'send', 'receive', 'queue', 'audit', 'trust', 'heartbeat', 'task_dispatch', 'task_status', 'task_cancel', 'task_retry', 'lease_status', 'metrics', 'message_verify', 'peer_verify', 'trust_inspect', 'runtime_start', 'runtime_stop', 'runtime_status', 'outbox_status', 'outbox_retry', 'deadletters_list', 'trace_inspect'],
  ...
  control: ['status', 'health', 'metrics', 'events', 'trace', 'sessions', 'doctor'],
};
...
const actionSchema = (family: McpFamily, action: string): z.ZodRawShape => {
  ...
  if (family === 'federation') { if (['node_status', 'node_drain', 'node_remove', 'heartbeat', 'lease_status', 'trust_inspect'].includes(action)) shape.nodeId = z.string().min(1); if (['task_status', 'task_cancel', 'task_retry', 'trace_inspect'].includes(action)) shape.taskId = z.string().min(1); if (action === 'node_register') { shape.nodeId = z.string().min(1).optional(); shape.name = z.string().min(1); shape.endpoint = z.string().min(1); shape.role = z.enum(['coordinator', 'scheduler', 'worker', 'hybrid']); shape.capabilities = z.array(z.string()); shape.trustLevel = z.enum(['UNTRUSTED', 'LIMITED', 'TRUSTED', 'ADMIN']).optional(); } if (action === 'task_dispatch') { shape.taskId = z.string().min(1); shape.requiredCapabilities = z.array(z.string()).optional(); shape.capabilities = z.array(z.string()).optional(); shape.priority = z.number().int().optional(); shape.locality = z.enum(['local', 'remote', 'any']).optional(); shape.nodeId = z.string().optional(); shape.trustLevel = z.enum(['UNTRUSTED', 'LIMITED', 'TRUSTED', 'ADMIN']).optional(); shape.approvedBy = z.string().optional(); shape.title = z.string().optional(); } if (action === 'message_verify' || action === 'peer_verify') { shape.message = z.record(z.unknown()); shape.secret = z.string().min(1); shape.keyId = z.string().optional(); } if (action === 'task_dispatch') { shape.sandbox = z.record(z.unknown()).optional(); shape.attemptId = z.string().optional(); } }
  if (family === 'system') shape.metadata = z.record(z.unknown()).optional();
  if (family === 'control' && action === 'trace') shape.executionId = z.string().min(1);
