...
function riskFor(family: McpFamily, action: string): McpRisk {
  ...
  if (['create', 'update', 'delete', 'compact', 'migrate', 'expire', 'spawn', 'pause', 'resume', 'stop', 'cancel', 'retry', 'submit', 'rebalance', 'run', 'write', 'register', 'destroy', 'scale', 'add_agent', 'remove_agent', 'replace_agent', 'delegate', 'handoff', 'topology_switch', 'transition'].includes(action)) return 'WRITE';
  return 'READ';
}
...
export class HelixMcpServer {
  ...
  readonly resources = ['helix://agents', 'helix://tasks', 'helix://scheduler', 'helix://swarm', 'helix://memory', 'helix://metrics', 'helix://events', 'helix://system', 'helix://goals', 'helix://plans', 'helix://orchestrations', 'helix://swarms', 'helix://swarm-collaboration'];
  ...
  private createSdkServer(): McpServer {
    ...
    const resourceData = async (uri: string): Promise<unknown> => { const actorId = process.env.HELIX_MCP_ACTOR ?? 'mcp-user'; if (!this.registry.authorization.check({ id: actorId, role: this.registry.authorization.role(actorId) }, 'READ')) throw new McpToolError('FORBIDDEN', 'resource authorization denied'); if (uri.endsWith('/agents')) return this.runtime.agents.list(); if (uri.endsWith('/swarms')) return this.runtime.swarms.list(); if (uri.endsWith('/swarm-collaboration')) return this.runtime.swarms.list().map((swarm) => ({ swarmId: swarm.id, graph: this.runtime.swarms.collaboration(swarm.id) })); if (uri.endsWith('/memory')) return this.runtime.memoryStats({ subject: actorId }); if (uri.endsWith('/metrics')) return this.runtime.telemetrySnapshot(); if (uri.endsWith('/events')) return { sequence: this.runtime.events.lastSequence }; if (uri.endsWith('/system')) return { provider: this.runtime.provider.name, memory: this.runtime.memory.constructor.name }; if (uri.endsWith('/scheduler')) return this.runtime.scheduler.list(); return { uri, available: true, protected: true }; };
