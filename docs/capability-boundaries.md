...
|---|---|---|
| Durable execution | JSONL append-only event store, ordering, idempotency, replay, snapshots, pause/resume/cancel/retry/checkpoint controls, and restart recovery scans | A production deployment still needs a reviewed multi-process store such as PostgreSQL or an equivalent event-log service |
...
| Security | Default-deny rules, approval records, budgets, explicit high-risk denial, optional API bearer authentication, request-size bounds, and per-client rate limiting | Secret vaults, OS/container sandboxes, formal threat-model verification, and third-party audits remain required before deployment |
