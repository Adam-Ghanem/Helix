...
- The verified artifact is exposed inside Bubblewrap only through a trusted read-only bind at `/plugin/worker.mjs`; scratch state remains under writable `/workspace`.
...
### Task 2: Trusted read-only binds and persistent isolated sandbox sessions
...
**Interfaces:**
- Extends `BubblewrapSandboxOptions` / `SandboxManagerOptions` with `readOnlyBinds?: Array<{ source: string; target: string }>`.
- Each source is absolute; each target is absolute and cannot overlap `/workspace`, `/home`, `/proc`, `/dev`, or `/tmp`.
...
- `BubblewrapSandbox.spawnSession()` uses the same isolation plan as one-shot execution and emits trusted `--ro-bind` mappings before command launch.
...
- [ ] **Step 1: Write failing tests** proving explicit read-only bind validation/plan generation, no-network/read-only/runtime constraints, line framing bounds, deterministic close/kill, and unsafe session labeling.
- [ ] **Step 2: Verify RED** on missing read-only bind/session APIs.
- [ ] **Step 3: Implement trusted bind validation.** Normalize sources, reject non-absolute targets and protected/writable target prefixes, and emit exact `--ro-bind source target` arguments without allowing untrusted plugin input to configure them.
- [ ] **Step 4: Implement minimal persistent session runner** with `spawn(..., shell:false)`, bounded incremental stdout line parser, bounded stderr accumulation, write backpressure handling, close grace timeout, and forced kill fallback.
- [ ] **Step 5: Run full CI** and preserve all one-shot sandbox tests.
- [ ] **Step 6: Review** for child-process leaks, unbounded buffering, inherited environment, target-path escape, and duplicate exit settlement.
...
- [ ] **Step 4: Implement sandbox launch contract.** Worker factory receives verified host artifact path and configures the sandbox read-only bind to `/plugin/worker.mjs`; the spawned command is absolute Node plus `/plugin/worker.mjs` only.
- [ ] **Step 5: Implement bounded restart.** Unexpected exit marks unhealthy; next call may relaunch until `maxRestarts=3`, then fails with an explicit open-circuit error. Explicit `stop()` does not consume restart budget.
- [ ] **Step 6: Run full CI and review** for request leaks, timer leaks, reentrancy, double resolution, artifact-path leakage, and circuit reset semantics.
...
- CLI worker sandbox factory uses `SandboxManager` with `allowUnsafeFallback` omitted/false and the verified artifact read-only bind.
...
- [ ] **Step 1: Write failing CLI tests** using a signed temporary manifest + worker fixture to prove relative entrypoint resolution, data-only install compatibility, strict isolation refusal when Bubblewrap is unavailable, read-only artifact bind creation, and network policy derivation without a wildcard bypass.
